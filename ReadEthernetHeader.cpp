...
ReadEthernetHeader::ReadEthernetHeader(const u_char* start_header){
	...
	ethernet_header_ = std::make_shared<const ETHHR> (*(reinterpret_cast<const ETHHR*> (const_cast<u_char*> (start_header))));
...
std::string ReadEthernetHeader::get_ethernet_type() const {return ether_type_;}
