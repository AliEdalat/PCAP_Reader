...
ReadTCPHeader::ReadTCPHeader(const u_char* start_header , int prev_header_size , int pkthdr_len){
	...
	std::unique_ptr<TCPHR> tcp_header_temp (reinterpret_cast<TCPHR*> (start_header_.get()));
	tcp_header_=tcp_header_temp;
	source_port_ = ntohs(tcp_header_->source);
	des_port_ = ntohs(tcp_header_->dest);
	std::unique_ptr<u_char> data_temp (start_header_.get() + sizeof(TCPHR));
	data_=data_temp;
